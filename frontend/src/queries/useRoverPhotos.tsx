...
function useRoverPhotos() {
  ...
  const sol = useMemo(() => state.sol ?? null, [state.sol])
  const earthDate = useMemo(() => state.earth_date ?? null, [state.earth_date])

  const res = useInfiniteQuery(
    ["photos", selectedRoverName, selectedCameraName, sol, earthDate],
    ({ pageParam }) =>
      fetchRoverData(selectedRoverName, selectedCameraName, pageParam, sol, earthDate),
    {
      enabled: !!selectedRoverName && (sol != null || earthDate != null),
